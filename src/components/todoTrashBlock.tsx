...
export const TodoTrashBlock: React.FC<ITodoTrashBlock> = (props) => {
  const dispatch = useDispatch();

  const [isShowMenu, setIsShowMenu] = useState<boolean>(false);

  const todoTasksArr = useSelector(
    (state: rootState) => state.todoAppSlice.tasks
  );
    ...
    >
      ...
      {isShowMenu && !props.isChild ? (
        <div className={todoStyles.todoHiddenMenus}>
          ...
          {todoTasksArr.find((el) => el.id === props.parentId) ? (
            <button
